...
export const register = catchAsyncError(async (req, res, next) => {
    ...
    if (!name || !email || !password || !enrollmentId || !file) {
        return next(new ErrorHandler("Please add all filds", 400))
    };

    let user = await User.findOne({ email });
    let Id = await User.findOne({ enrollmentId });
    if (user) {
        return next(new ErrorHandler("User Already Exists", 409))
    };
    if(Id){
        return next(new ErrorHandler("Enrollment Id Already Exists", 409));
    };
...
export const getAllUsers = catchAsyncError(async (req, res, next) => {
    ...
    res.status(200).json({
        success: true,
        message: `${users.length} Record Found`,
        users,
    });
